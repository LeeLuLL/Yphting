layui.config({
	...
	$.ajax({
		...
		success : function(data){
			console.log(data);
			for(var i in data){
				var option =$("<option value="+data[i].fmid+">"+data[i].fmname+"</option>");
				$("#lo").append(option);
			}
	...
	$("body").on("click",".news_del",function(){  //删除
		...
		layer.confirm('确定删除此信息？',{icon:3, title:'提示信息'},function(index){
			 var f=false;
		        ...
		        var ck=document.getElementsByName("checked");//得到一组checkbox  相当于数组
		...
		if(that.list.length != 0){
			...
			for(var i=0;i<that.list.length;i++){
				dataHtml += '<tr>'
			    	...
			    	+'<td>'+that.list[i].fmid+'</td>'
			    	+ '<td>'+that.list[i].fmname+'</td>'
			    	+ '<td>'+that.list[i].content+'</td>'
			    	+'<td>'+that.list[i].releasetime+'</td>';
			    	if(that.list[i].newsStatus == "0"){
			    		dataHtml += '<td style="color:#f00">已审核</td>';
			    	}else{
			    		dataHtml += '<td>未审核</td>';
			    	}
			    	dataHtml += 
...
/*		function renderDate(data,curr){
			...
			if(currData.length != 0){
				for(var i=0;i<currData.length;i++){
					...
					dataHtml += '<tr>'
			    	+'<td><input type="checkbox" name="checked" lay-skin="primary" lay-filter="choose"></td>'
			    	+'<td>'+currData[i].postid+'</td>';
			    	if(currData[i].complainttype == "待审核"){
			    		dataHtml += '<td style="color:#f00">'+currData[i].complainttype+'</td>';
			    	}else{
			    		dataHtml += '<td>'+currData[i].complainttype+'</td>';
			    	}
