...
public class RuZhuServlet {
	...
	public String merchantApply(HttpSession session, User user, String serviceID, MultipartFile shopimgData,
		...
		if (600 >= bond) { // 如果当前登录用户的金额足够缴纳保证金
			...
			if (biz.merchantApply(user, bond) > 0) {// 商家入驻受影响行数
				...
				//GoldNotes gold = new GoldNotes(loginUser.getUserid(), "商家入驻缴纳保证金", Float.parseFloat("-500"), 1);
