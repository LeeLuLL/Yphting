...
import com.accp.t4.action.llAction.config.AlipayConfig;
...
//		//设置请求参数
		...
		String out_trade_no =orderBiz.getGoldnotesId()+"";
		//付款金额，必填
	...
	public String returnUrl( HttpSession session,HttpServletRequest request, HttpServletResponse response)
		...
		if(signVerified) {
            ...
            orderBiz.updateUserMoney(mount, userId);
            orderBiz.updateGoldnotes(Integer.parseInt(out_trade_no), 4);
		}else {
			request.setAttribute("reason", "验签失败");
		}
		request.setAttribute("signVerified", signVerified);
		return "redirect:/grzx/money?num=1";
	}
